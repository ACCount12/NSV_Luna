				...
				if(1)
					...
					//	world << "MOVE"
						...
						for(var/obj/machinery/door/unpowered/shuttle/D in start_location) // Made doors close when pod launches, too --Mloc
							if(!D.density)
								D.close()
						start_location.move_contents_to(end_location, /turf/simulated/floor/engine/vacuum)
						location = 0
						direction = 2
...
proc/CreateShuttles() //Would do this via config, but map changes are rare and need source code anyway
	var/datum/shuttle/pod1 = new /datum/shuttle("Escape pod 1","/area/shuttle/escape_pod1/station","/area/shuttle/escape_pod1/transit","/area/shuttle/escape_pod1/centcom")
	var/datum/shuttle/pod2 = new /datum/shuttle("Escape pod 2","/area/shuttle/escape_pod2/station","/area/shuttle/escape_pod2/transit","/area/shuttle/escape_pod2/centcom")
